...
import AssignAsset from "./AssignAsset"; // <= même composant que sur la table
...
export default function AssetDetail() {
    ...
    const exitRepair = async () => {
        ...
        if (raw && raw.trim() !== "") {
            ...
            if (Number.isNaN(parsed) || parsed < 0) { alert("Montant invalide."); return; }
    ...
    const retireAsset = async () => {
        ...
        if (!confirm("Confirmer le retrait définitif de ce matériel ?")) return;
    ...
    const saveEdit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const priceNum =
                edPrice.trim() === "" ? null :
                    Number.isNaN(Number(edPrice.replace(",", "."))) ? (()=>{ throw new Error("Prix invalide"); })() :
    ...
    if (loading) {
        return (
            <main className="shell"><div className="shell-inner"><p style={{ padding: 24 }}>Chargement…</p></div></main>
        );
    }
    if (!asset) {
        return (
            <main className="shell"><div className="shell-inner"><p style={{ padding: 24 }}>Matériel introuvable</p></div></main>
        );
    }

    return (
        <motion.main className="shell" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
            <div className="shell-inner">
                ...
                <button
                    ...
                    style={{
                        ...
                        fontSize: 24, lineHeight: 1, display: "grid", placeItems: "center",
                        cursor: "pointer", boxShadow: "0 4px 12px rgba(0,0,0,.06)", zIndex: 3
                    }}
                ...
                <motion.div className="shell-body" variants={sweep} initial="initial" animate="animate" exit="exit">
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingRight: 48 }}>
                        ...
                        <span style={{
                            ...
                            background:
                                asset.status === "assigned" ? "var(--brand)" :
                                    asset.status === "repair"   ? "#b98b46" :
                                        asset.status === "retired"  ? "#eee" :
                                            "#f4f1ee"
                        }}>
              {asset.status}
            </span>
                    ...
                    {/* Infos principales */}
                    <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 6 }}>
                        ...
                        <Info label="Prix d’achat" value={asset.purchase_price != null ? asset.purchase_price.toFixed(2) : "—"} />
                    ...
                    {isAdmin && (
                        <section style={{ borderTop: "1px solid var(--line)", paddingTop: 12 }}>
                            ...
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                                {asset.status !== "repair" && <button className="pill" onClick={sendToRepair}>Mettre en réparation</button>}
                                {asset.status === "repair" && <button className="pill" onClick={exitRepair}>Sortie de réparation</button>}
                                {asset.status !== "retired" && <button className="pill" onClick={retireAsset}>Retirer définitivement</button>}
                            </div>
                        </section>
                    )}
                            ...
                            <ul style={{ margin: 0, paddingLeft: 16 }}>
                                {timeline.map((ev) => (
                                    <li key={ev.event_id}>
                                        ...
                                        {ev.notes ? ` — ${ev.notes}` : ""}
                                        {ev.event_type === "maintenance" && ev.repair_cost != null
                                            ? ` — coût: ${ev.repair_cost.toFixed(2)}`
                                            : ""}
                                    </li>
                                ))}
                            </ul>
                ...
                <Modal open={assignOpen} onClose={closeAssign} title={`Attribuer : ${asset.label}`}>
                    <AssignAsset
                        assetId={asset.id}
                        onDone={async () => { closeAssign(); await load(); }}
                    />
                </Modal>
                ...
                <Modal open={returnOpen} onClose={closeReturn} title={`Retourner : ${asset.label}`}>
                    ...
                    <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
                        ...
                        <button className="pill" onClick={async ()=>{ await returnAsset(); closeReturn(); }} type="button">Confirmer</button>
                    </div>
                </Modal>
                ...
                <Modal open={editOpen} onClose={() => setEditOpen(false)} title={`Modifier : ${asset.label}`}>
                    <form onSubmit={saveEdit} className="form-grid">
                        ...
                        <div className="span-2 modal-actions">
                            <button type="button" className="pill pill--muted" onClick={()=>setEditOpen(false)}>Annuler</button>
                            <button className="pill" disabled={savingEdit}>{savingEdit ? "Enregistrement…" : "Enregistrer"}</button>
                        </div>
                    </form>
                </Modal>
            </div>
