...
export default function RouteGuard({ children }: { children: React.ReactNode }) {
  ...
  const isPublicRoute = /^\/asset\/\d+$/.test(location.pathname);
  ...
  if (isPublicRoute) {
    return (
      <AnimatePresence mode="wait">
        <motion.div key="app" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
          {children}
        </motion.div>
      </AnimatePresence>
    );
        ...
        <div className="login-wrap">
          ...
          <section className="right-col">
            ...
            >
              ...
              {/* Bouton Google identique au composant Supabase Auth UI */}
              <button
                onClick={() => auth.signInWithGoogle(location.pathname)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
