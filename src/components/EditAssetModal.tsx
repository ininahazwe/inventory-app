...
export function EditAssetModal({ open, onClose, asset, onSaved }: { open: boolean; onClose: () => void; asset: any; onSaved: () => Promise<void> | void; }) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await api.put(`/assets/${asset.id}`, { serial_no: edSerial.trim() || null, category_id, purchased_at: edPurchasedAt || null, purchase_price: priceNum, supplier: edSupplier.trim() || null, warranty_end: edWarrantyEnd || null, notes: edNotes.trim() || null });
      if (error) throw new Error(error);
