...
export default function Autocomplete({
  ...
  onSelect?: (v: string) => void;  // ✅ Nouveau prop optionnel
  ...
  // ✅ Quand on sélectionne un item
  const handleSelect = (selectedValue: string) => {
    onChange(selectedValue);
    setQ(selectedValue);
    setOpen(false);
    onSelect?.(selectedValue);  // ✅ Appeler le callback
  };
        ...
        <div className="auto-list">
          {opts.map(o => (
            <div
              key={o}
              className="auto-item"
              onMouseDown={() => handleSelect(o)}  // ✅ Utiliser handleSelect
            >
              {o}
            </div>
          ))}
