...
export default function AssigneeForm({ assignee, onSaved, onCancel }: { assignee: Assignee; onSaved: () => void; onCancel: () => void; }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (new_email && !/^\S+@\S+\.\S+$/.test(new_email)) return alert('Email invalide');
    setBusy(true);
    const { error } = await rpc('assignees_rename', { old_email: assignee.email, old_name: assignee.full_name, new_name, new_email });
