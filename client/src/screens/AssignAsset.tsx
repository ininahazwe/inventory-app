...
export default function AssignAsset({ assetId, onDone }: { assetId: number; onDone?: () => void }) {
  ...
  async function fetchPeopleEmails(q: string) {
    const { data } = await api.get<string[]>(`/assignments/autocomplete/emails${q ? `?q=${encodeURIComponent(q)}` : ''}`);
    return data ?? [];
  }

  const submit = async (e: React.FormEvent) => {
    e.preventDefault(); setLoading(true); setErr(null);
    const { error } = await rpc('assign_asset', { p_asset_id: assetId, p_name: name, p_email: email || null });
    setLoading(false);
    if (error) { setErr(error); return; }
    setName(''); setEmail(''); onDone?.();
  };
